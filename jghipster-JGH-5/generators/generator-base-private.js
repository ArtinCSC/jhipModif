...
module.exports = class extends Generator {
    ...
    checkGo() {
        ...
        exec('go version', (err, stdout, stderr) => {
            if (err) {
                this.warning('Go is not found on your computer.');
            } else {
                const goVersion = semver.clean(stdout);
                const goFromPackageJson = packagejs.engines.go;
                if(!semver.satisfies(goVersion, goFromPackageJson)){
                    this.warning(
                        `Your Go version is too old (${goVersion}). You should use at least Go ${chalk.bold(goFromPackageJson)}`
                );
                }
            }
            done();
