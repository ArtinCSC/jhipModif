...
function askForGoOrSpring(meta) {
    if (!meta && this.existingProject) return;
    const DEFAULT_TYPE = 'spring-boot';

    const frameworkTypeChoices = [
        {
            value: DEFAULT_TYPE,
            name: 'Spring-Boot framework(recommended)'
        },
        {
            value: 'GoLang',
    ...
    const PROMPT = {
        ...
        message: `Which ${chalk.yellow('*type*')} of framework would you like to use?`,
        choices: frameworkTypeChoices,
    ...
    promise.then(prompt => {
        if (prompt.frameworkType === 'spring-boot') {
            this.frameworkType = DEFAULT_TYPE;
        } else if (prompt.frameworkType === 'GoLang') {
            this.frameworkType = 'GoLang';
        }
        done();
    });
}
